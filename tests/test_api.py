...
class TestTaskAPI:
    ...
    def test_create_task_without_description(self, authenticated_client):
    ...
    def test_get_tasks(self, authenticated_client):
        ...
        for task in tasks:
            authenticated_client.post("/api/tasks/", json=task)
    ...
    def test_update_task(self, authenticated_client):
        ...
        response = authenticated_client.put(f"/api/tasks/{task_id}", json=update_data)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_nonexistent_task(self, authenticated_client):
        ...
        response = authenticated_client.put("/api/tasks/9999", json=update_data)
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_task_pagination(self, authenticated_client):
        ...
        # Test with skip
        response = authenticated_client.get("/api/tasks/?skip=5&limit=5")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.json()) == 5
